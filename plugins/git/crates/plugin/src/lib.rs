...
use inexor_rgf_model_git as model_git;
...
pub fn construct_plugin() -> Result<Arc<dyn Plugin>, PluginLoadingError> {
    ...
    let plugin = Provider::<dyn GitPlugin>::create(container);
...
extern "C" fn get_dependencies() -> Vec<PluginDependency> {
    vec![
        ...
        PluginDependency::new("inexor-rgf-plugin-file", ">=0.9.0, <010.0"),
        PluginDependency::new("inexor-rgf-plugin-http", ">=0.9.0, <010.0"),
        PluginDependency::new("inexor-rgf-plugin-trigger", ">=0.9.0, <010.0"),
    ]
}

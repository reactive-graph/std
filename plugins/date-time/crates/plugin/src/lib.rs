...
use inexor_rgf_model_date_time as model_date_time;
...
use crate::plugin::DateTimePlugin;
...
pub fn construct_plugin() -> Result<Arc<dyn Plugin>, PluginLoadingError> {
    ...
    let plugin = Provider::<dyn DateTimePlugin>::create(container);
...
extern "C" fn get_dependencies() -> Vec<PluginDependency> {
    vec![
        PluginDependency::new("inexor-rgf-plugin-result", ">=0.8.0, <0.9.0"),
        PluginDependency::new("inexor-rgf-plugin-trigger", ">=0.8.0, <0.9.0"),
    ]
}

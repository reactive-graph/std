...
impl Plugin for NumericPluginImpl {
    async fn activate(&self) -> Result<(), PluginActivationError> {
        ...
        let entity_behaviour_registry = self.context.get_entity_behaviour_registry();
        entity_behaviour_registry.register_all(&NUMERIC_OPERATIONS_F64.get_factories()).await;
        entity_behaviour_registry.register_all(&NUMERIC_OPERATIONS_I64.get_factories()).await;
        entity_behaviour_registry.register_all(&NUMERIC_GATES_F64.get_factories()).await;
        Ok(())
    }

    async fn deactivate(&self) -> Result<(), PluginDeactivationError> {
        let entity_behaviour_registry = self.context.get_entity_behaviour_registry();
        entity_behaviour_registry.unregister_all(&NUMERIC_GATES_F64.to_entity_behaviour_tys()).await;
...
// use crate::di::*;
// use inexor_rgf_model_base::NamedProperties;

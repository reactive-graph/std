...
impl LogicalEntityBehaviourProvider for LogicalEntityBehaviourProviderImpl {
    ...
    fn remove_logical_operation(&self, entity_instance: Arc<ReactiveEntityInstance>) {
        if self.logical_operations.0.write().unwrap().remove(&entity_instance.id).is_some() {
    ...
    fn remove_logical_gate(&self, entity_instance: Arc<ReactiveEntityInstance>) {
        if self.logical_gates.0.write().unwrap().remove(&entity_instance.id).is_some() {
            entity_instance.remove_behaviour(entity_instance.type_name.as_str());
    ...
    fn remove_toggle(&self, entity_instance: Arc<ReactiveEntityInstance>) {
        if self.toggles.0.write().unwrap().remove(&entity_instance.id).is_some() {
    ...
    fn remove_trigger(&self, entity_instance: Arc<ReactiveEntityInstance>) {
        if self.triggers.0.write().unwrap().remove(&entity_instance.id).is_some() {
    ...
    fn remove_if_then_else(&self, entity_instance: Arc<ReactiveEntityInstance>) {
        if self.if_then_else_behaviours.0.write().unwrap().remove(&entity_instance.id).is_some() {
    ...
    fn remove_by_id(&self, id: Uuid) {
        if self.logical_operations.0.write().unwrap().contains_key(&id) && self.logical_operations.0.write().unwrap().remove(&id).is_some() {
            debug!("Removed behaviour logical_operation from entity instance {}", id);
        }
        if self.logical_gates.0.write().unwrap().contains_key(&id) && self.logical_gates.0.write().unwrap().remove(&id).is_some() {
            debug!("Removed behaviour logical_gates from entity instance {}", id);
        }
        if self.toggles.0.write().unwrap().contains_key(&id) && self.toggles.0.write().unwrap().remove(&id).is_some() {
            debug!("Removed behaviour {} from entity instance {}", TOGGLE, id);
        }
        if self.triggers.0.write().unwrap().contains_key(&id) && self.triggers.0.write().unwrap().remove(&id).is_some() {
            debug!("Removed behaviour {} from entity instance {}", TRIGGER, id);
        }
        if self.if_then_else_behaviours.0.write().unwrap().contains_key(&id) && self.if_then_else_behaviours.0.write().unwrap().remove(&id).is_some() {
